...
####### Exponential filter #######
# Generate certain amount of samples for each type
def generate_samples_par_exp(M, R, dt, dynamics_fun, around_z_angles, eta_1, sigma, theta_r, K, L, sample_dt, delay_dt, D_min, D_max, v_min, v_max,\
                             P, steps_r, D_min_r, D_max_r, scal, N1, N2, N3, N4, M1, M2, M3, M4, NNs,\
                             set_number, savepath, num_cores):
    """
    Args:
    ...
    dt: time step (sec)
    dynamics_fun: predefined dynamics of the object, return the accelerations (/sec^2)
    around_z_angles: rotation angles around the z axis
    eta_1: random force added on the ball (/sec^2)
    sigma: noise added to images
    theta_r: half of the receptive field width (rad)
    K: K*K is the total number of elements.
    ...
    f0.write('dt = {}: time step (sec)\n'.format(dt))
    f0.write('dynamics_fun = '+dynamics_fun.__name__+': predefined dynamics of the object, return the accelerations (m/sec^2)\n')
    f0.write('around_z_angles = {}: rotation angles around the z axis \n'.format(around_z_angles))
    f0.write('eta_1 = {}: random force added on the ball (m/sec^2)\n'.format(eta_1))
    f0.write('sigma = {}: noise added to images\n'.format(sigma))
    f0.write('theta_r = {:.4g}: half of the receptive field width (rad)\n'.format(theta_r))
    f0.write('K = {}: K*K is the total number of elements\n'.format(K))
    ...
    Rs = np.array([R])
    generate_trajectory_hit(\
        R, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, N1+M1, NNs, savepath, set_number, datatype)
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp)\
             (n1, N1, M1, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, savepath, set_number, datatype)\
             for n1 in range(1, N1+M1+1))
    print('{} trainging samples have been generated!'.format(N1))
    print('{} testing samples have been generated!'.format(M1))
    ...
    Rs = np.array([R])
    generate_trajectory_miss(\
        R, D_min, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, N2+M2, NNs, savepath, set_number, datatype)
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp)\
             (n2, N2, M2, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, savepath, set_number, datatype)\
             for n2 in range(1, N2+M2+1))
    print('{} trainging samples have been generated!'.format(N2))
    print('{} testing samples have been generated!'.format(M2))
    ...
    Rs = np.array([R])
    generate_trajectory_retreat(\
        R, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, N3+M3, NNs, savepath, set_number, datatype)
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp)\
             (n3, N3, M3, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, savepath, set_number, datatype)\
             for n3 in range(1, N3+M3+1))
    print('{} trainging samples have been generated!'.format(N3))
    print('{} testing samples have been generated!'.format(M3))
    ...
    datatype = 'rotation'
    Rs = np.random.random(P) * R
    generate_trajectory_rot(D_min_r, D_max_r, P, steps_r, dt, scal, N4+M4, NNs, savepath, set_number, datatype)
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp)\
             (n4, N4, M4, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, savepath, set_number, datatype)\
             for n4 in range(1, N4+M4+1)) 
    print('{} training samples have been generated!'.format(N4))
    print('{} testing samples have been generated!'.format(M4))
...
# Generate one sample, in parallel
def generate_sample_par_exp(n, N0, M0, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
                        dt, sample_dt, delay_dt, savepath, set_number, datatype):
    ...
    traj_list = np.load(savepath+'set_{}/'.format(set_number)+'other_info/'+datatype+'/trajectories/traj_list_{}.npy'\
                        .format(n), allow_pickle=True)
    intensities_sample_cg_list, UV_flow_sample_list, distance_sample_list \
        = generate_one_sample_exp(M, Rs, traj_list, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
                              dt, sample_dt, delay_dt)
    if n <= N0:
        np.save(savepath+'set_{}/'.format(set_number)+'training/'+datatype\
                +'/intensities_samples_cg/intensities_sample_cg_list_{}'\
...
def generate_one_sample_exp(\
    M, Rs, traj_list, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L, dt, sample_dt, delay_dt):
    """
    Args:
    M: # of lplc2 units
    Rs: radii of the balls 
    ...
    for nn in range(NN):
        ...
        for step in range(steps):
            ...
            if step > 0 and step % sample_step == 0:
                # the current frame
                pos = traj[step]
                cf, cf_raw, hit = opsg.get_one_intensity(M, pos, Rs, theta_r, theta_matrix, coord_matrix, K, L, pad, around_z_angles, sigma)
                ...
                UV_flow_sample.append(UV_flow)
                distance.append(Ds)
        # intensities_sample_list.append(intensities_sample)
        intensities_sample_cg_list.append(intensities_sample_cg)
        UV_flow_sample_list.append(UV_flow_sample)
        distance_sample_list.append(distance)
...
def generate_trajectory_hit(\
    R, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, Ns, NNs, savepath, set_number, datatype, suffix=''):
    """
    Args:
    R: radius of the ball 
...
def generate_trajectory_miss(\
    ...
    while n <= Ns:
        ...
        while nn <= NNs:
            ...
            traj, distance =\
                generate_one_trajectory_miss(R, D_max, dt, dynamics_fun, eta_1, x, y, z, vx, vy, vz)
            if len(distance) > 0:
                D = np.min(distance)
            else:
                D = 0
...
def generate_trajectory_retreat(\
    ...
    while n <= Ns:
        ...
        while nn <= NNs:
            ...
            x, y, z, vx, vy, vz = generate_init_condition(0, R, v_min, v_max)
            traj, distance = \
                generate_one_trajectory_retreat(R, D_max, dt, dynamics_fun, eta_1, x, y, z, vx, vy, vz)
            if len(traj) * dt > delay_dt:
                traj_list.append(traj)
                distance_list.append(distance)
                nn = nn + 1
...
def generate_trajectory_rot(D_min, D_max, P, steps, dt, scal, Ns, NNs, savepath, set_number, datatype):
    """
    Args:
    ...
    while n <= Ns:
        ...
        while nn <= NNs:
            ...
            za = np.cos(theta_s)
            traj = generate_one_trajectory_rot(D_min, D_max, P, steps, dt, xa, ya, za, scal)
            traj_list.append(traj)
            nn = nn + 1
        np.save(savepath+'set_{}/'.format(set_number)+'other_info/'+datatype+'/trajectories/traj_list_{}'\
                .format(n), traj_list)
...
def generate_one_trajectory_hit(R, D_max, dt, dynamics_fun, eta_1, x, y, z, vx, vy, vz):
    """
    Args:
    R: radius of the ball 
    D_max: maximum distance from the origin 
...
def generate_one_trajectory_miss(R, D_max, dt, dynamics_fun, eta_1, x, y, z, vx, vy, vz):
    """
    Args:
    R: radius of the ball 
    D_max: maximum distance from the origin 
...
def generate_one_trajectory_retreat(R, D_max, dt, dynamics_fun, eta_1, x, y, z, vx, vy, vz):
    """
    Args:
    R: radius of the ball 
    D_max: maximum distance from the origin 
...
def generate_one_trajectory_rot(D_min_r, D_max_r, P, steps, dt, xa, ya, za, scal=200, random_z=True, fixed_z=0):
    """
    Args:
    D_min_r: minimum distance of the balls in the rotation scenes
    D_max_r: maximum distance of the balls in the rotation scenes
    ...
    pos = []
    for p in range(P):
        D = D_min_r + (D_max_r - D_min_r) * np.random.random()
        theta_s = np.arccos(2*np.random.random()-1)
        phi_s = 2. * np.pi * np.random.random()
...
####################################################################
####### Generate samples with noisy or rotational background #######
####################################################################

# Generate certain amount of samples for each type
def generate_samples_par_exp_with_noisy_rot(M, R, dt, dynamics_fun, around_z_angles, eta_1, sigma, theta_r, K, L, sample_dt, delay_dt, D_min, D_max, v_min, v_max,\
    ...
    datatype = 'hit'
    Rs = np.ones(P+1)
    Rs[:P] = np.random.random(P) * R
    generate_trajectory_hit(\
        R, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, N1+M1, NNs, savepath, set_number, datatype, suffix='_wbn')
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp_with_noisy_rot)\
             (n1, N1, M1, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, D_min_r, D_max_r, P, scal, hl, savepath, set_number, datatype)\
             for n1 in range(1, N1+M1+1))
    print('{} trainging samples have been generated!'.format(N1))
    print('{} testing samples have been generated!'.format(M1))
    ...
    Rs[:P] = np.random.random(P) * R
    generate_trajectory_miss(\
        R, D_min, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, N2+M2, NNs, savepath, set_number, datatype, suffix='_wbn')
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp_with_noisy_rot)\
             (n2, N2, M2, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, D_min_r, D_max_r, P, scal, hl, savepath, set_number, datatype)\
    ...
    Rs[:P] = np.random.random(P) * R
    generate_trajectory_retreat(\
        R, D_max, v_min, v_max, dt, delay_dt, dynamics_fun, eta_1, N3+M3, NNs, savepath, set_number, datatype, suffix='_wbn')
    Parallel(n_jobs=num_cores)\
            (delayed(generate_sample_par_exp_with_noisy_rot)\
             (n3, N3, M3, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
              dt, sample_dt, delay_dt, D_min_r, D_max_r, P, scal, hl, savepath, set_number, datatype)\
...
# Generate one sample, in parallel
def generate_sample_par_exp_with_noisy_rot(n, N0, M0, M, Rs, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
                                           dt, sample_dt, delay_dt, D_min_r, D_max_r, P, scal, hl, savepath, set_number, datatype):

    print('{} out of {} are Finished!'.format(n, N0+M0), end='\r')
    
    traj_list = np.load(savepath+'set_{}/'.format(set_number)+'other_info/'+datatype+'/trajectories/traj_list_{}_wbn.npy'\
                        .format(n), allow_pickle=True)
    _, intensities_sample_cg_list, UV_flow_sample_list, distance_sample_list \
        = generate_one_sample_exp_with_noisy_rot(M, Rs, traj_list, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L,\
                                                 dt, sample_dt, delay_dt, D_min_r, D_max_r, P, scal, hl)
    if n <= N0:
        np.save(savepath+'set_{}/'.format(set_number)+'training/'+datatype\
                +'/intensities_samples_cg/intensities_sample_cg_list_{}_wbn'\
                .format(n), intensities_sample_cg_list)
        np.save(savepath+'set_{}/'.format(set_number)+'training/'+datatype\
                +'/UV_flow_samples/UV_flow_sample_list_{}_wbn'\
                .format(n), UV_flow_sample_list)
    else:
        np.save(savepath+'set_{}/'.format(set_number)+'testing/'+datatype\
                +'/intensities_samples_cg/intensities_sample_cg_list_{}_wbn'\
    ...
    if datatype != 'rotation':
        np.save(savepath+'set_{}/'.format(set_number)+'other_info/'+datatype\
                +'/distances/sample/distance_sample_list_{}_wbn'\
                .format(n), distance_sample_list)
        
        
# Generate one sample of optical stimulus        
def generate_one_sample_exp_with_noisy_rot(\
    M, Rs, traj_list, around_z_angles, sigma, theta_r, theta_matrix, coord_matrix, space_filter, K, L, dt, sample_dt, delay_dt, D_min_r, D_max_r, P, scal, hl):
    """
    Args:
    M: # of lplc2 units
    Rs: radii of the balls 
    ...
    for nn in range(NN):
        traj_b = traj_list[nn]
        traj = generate_one_trajectory_with_noisy_rot(D_min_r, D_max_r, P, traj_b, dt, scal, hl)
        steps = len(traj)
        intensities_sample = []
        intensities_sample_cg = []
        UV_flow_sample = []
    ...
    return intensities_sample_list, intensities_sample_cg_list, UV_flow_sample_list, distance_sample_list


# Generate one trajectory with noisy rotation background    
def generate_one_trajectory_with_noisy_rot(D_min_r, D_max_r, P, traj_b, dt, scal=200, hl=0.2, random_z=True, fixed_z=0):
    """
    Args:
    D_min_r: minimum distance of the balls in the rotation scenes
    D_max_r: maximum distance of the balls in the rotation scenes
    P: # of balls
    traj_b: trajectories of a ballistic motion
    dt: time step (sec)
    scal: scale of the rotaion, in degrees
    hl: half life
    random_z: whether randomly select the rotation speed around 'z' (rotated z, not the original)
    ...
    gamma1 = np.log(2.) / hl # half-life is 0.2 second
    e_factor = np.exp(-gamma1*dt)

    for step in range(steps):
        pos_all = np.concatenate((pos, traj_b[step]))
        traj.append(pos_all)

        theta_s = np.arccos(2*np.random.random()-1)
        phi_s = 2. * np.pi * np.random.random()
        xa = e_factor * xa + (1-e_factor) * np.sin(theta_s) * np.cos(phi_s)
        ya = e_factor * ya + (1-e_factor) * np.sin(theta_s) * np.sin(phi_s)
        za = e_factor * za + (1-e_factor) * np.cos(theta_s)
        angle = opsg.get_xy_angles(xa, ya, za)
        pos_r = opsg.get_rotated_coordinates(-angle, pos) # transform into the frame of reference of the rotation

        if random_z:
            around_z = e_factor * around_z + (1-e_factor) * np.random.normal(scale=scal*dt)
        else:
            around_z = fixed_z*dt
        r = R3d.from_euler('ZYX', [around_z, 0, 0], degrees=True)
        pos_r = r.apply(pos_r)
        ...
        for t in range(steps):
            pos_b_r = opsg.get_rotated_coordinates(-angle, traj_b[t])
            pos_b_r = r.apply(pos_b_r)
            traj_b[t] = opsg.get_rotated_coordinates_rev(angle, pos_b_r)
