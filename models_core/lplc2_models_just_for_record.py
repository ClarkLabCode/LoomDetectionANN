...
"""
This module contains functions that define the LPLC2 models with four excitatory units (directional motion signals as inputs), 
four individual inhibitory units (directional motion signals as inputs), and one intensity unit.
"""
...
def get_output_LPLC2(args, weights_e, weights_i, weights_intensity, intercept_e, intercept_i, UV_flow_t, intensity_t):
    """
    This function takes the flow field and intensity as inputs, and outputs the sum of the responses 
    ...
    weights_i: tensor, weights for inhibitory units. 
               ...
               shape = K*K by 4 (without rotation symmetry), where
               for axis=1, 0: right motion, 1: left motion, 2: up motion, 3: down motion
    weights_intensity: tensor, weights for intensity, shape = K*K.
    ...
    intensity_t: tensor, intensity at time point t, shape = batch_size by M by K*K
    ...
    # add intensity if needed
    if args['use_intensity']:
        input_intensity_t = tf.tensordot(intensity_t, weights_intensity, axes=[[2], [0]])
        output_t = output_t + input_intensity_t
...
def get_output_LPLC2_T(args, weights_e, weights_i, weights_intensity, intercept_e, intercept_i, UV_flow, intensity, tau_1):
    """
    This function takes the flow field and intensity as inputs, and outputs the sum of the responses 
    ...
    intensity: tensor, intensity, shape = steps by batch_size by M by K*K
    ...
    output_T = tf.map_fn(lambda x:\
        get_output_LPLC2(args, weights_e, weights_i, weights_intensity, intercept_e, intercept_i, x[0], x[1]), \
            (UV_flow, intensity), dtype=(tf.float32, tf.float32)) # shape = steps by batch_size
...
def get_loss_and_prob(args, weights_e, weights_i, weights_intensity, intercept_e, intercept_i, UV_flow, intensity, \
    ...
    """
    output_T = get_output_LPLC2_T(args, weights_e, weights_i, weights_intensity, \
        intercept_e, intercept_i, UV_flow, intensity, tau_1) # shape = steps by batch_size
...
def train_and_test_model(args, train_flow_files, train_labels, test_flow_files, test_labels, \
    ...
    train_intensity_snapshots: snapshot traning samples, intensity
    ...
    valid_intensity_snapshots = train_intensity_snapshots[int((1-validation_portion)*len(train_intensity_snapshots)):]
    train_intensity_snapshots = train_intensity_snapshots[:int((1-validation_portion)*len(train_intensity_snapshots))]
    ...
    if args['use_intensity']:
        intensity = tf.compat.v1.placeholder(tf.float32, [None, None, M, K*K], name='intensity')
    else:
        intensity = None
    ...
    with tf.compat.v1.variable_scope('with_inhibitory'):
        ...
        positive_initializer_intensity = tf.compat.v1.keras.initializers.TruncatedNormal(mean=0.002, stddev=0.001)
        ...
        # intensity weights
        if args['use_intensity']:
            weights_intensity = tf.Variable(positive_initializer_intensity([K*K]), name='weights_intensity')
        else:
        ...
        if args['fine_tune_intercepts_and_b']:
            ...
            intercept_e = tf.Variable(zero_initializer([1]), name='intercept_e')
            intercept_i = tf.Variable(zero_initializer([1]), name='intercept_i')
    ...
    with tf.compat.v1.Session() as sess:
        ...
        if args['use_intensity']:
            np.save(args['save_path']+'initial_weights_intensity', weights_intensity.eval())
    ...
    loss, probabilities = \
    get_loss_and_prob(args, weights_e, weights_i, weights_intensity, intercept_e, intercept_i, UV_flow, intensity, \
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for epoch in range(N_epochs):
            ...
            for sample_i in range(N_train):
                ...
                intensity_i = train_intensity_snapshots[sample_i].copy()
                ...
                if args['use_intensity']:
                    ...
                    sess.run([opt_with_clip, loss, probabilities], 
                        {UV_flow:UV_flow_i, labels:labels_i, intensity:intensity_i, step_weights:step_weights_i})
                else:
                    # shape = _, 0, batch_size
                    _, loss_i, probabilities_i = \
                    sess.run([opt_with_clip, loss, probabilities], 
                ...
                if args["save_intermediate_weights"] and counter_1 % args["save_steps"] == 1:
                    ...
                    if args['use_intensity']:
                        np.save(temp_dir+'trained_weights_intensity', weights_intensity_out)
            ...
            for sample_i in range(N_valid):
                ...
                intensity_i = valid_intensity_snapshots[sample_i].copy()
        ...
        if args['use_intensity']:
            np.save(args['save_path'] + 'trained_weights_intensity', weights_intensity_out)
        ...
        for sample_i, UV_flow_file_i, labels_i, step_weights_i in \
            ...
            if args['use_intensity']:
                UV_flow_i, step_weights_i, labels_i =\
                    hpfn.load_compressed_dataset(args, UV_flow_file_i, labels_i, step_weights_i)
                intensity_i, step_weights_i, labels_i =\
                    hpfn.load_compressed_dataset(args, args['train_intensities'][sample_i], labels_i, step_weights_i)
