...
def get_loss_and_prob(args, weights_e, weights_i, intercept_e, intercept_i, UV_flow, \
    ...
    logits = tf.abs(a) * output_T # shape = steps by batch_size
    ...
    # probability of hit
    probabilities = 2*(tf.nn.sigmoid(logits)-0.5) # shape = steps by batch_size
...
def cross_entropy_loss(logits, labels, step_weights):
    ...
    """
    p_pred = 2 * (tf.math.sigmoid(logits) - 0.5)
    celoss = tf.math.multiply(labels, -tf.math.log(p_pred+1e-30)) + \
        tf.math.multiply(1-labels, -tf.math.log(1-p_pred+1e-30))
